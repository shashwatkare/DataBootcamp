...
|18| AmericanDream | __Pathway to Homeownership__ <br>Website: http://minihome.myqnapcloud.com:5001/<br>Used data including <br>1. `average 30-year fixed mortgage rate by state`<br>2. `state median house listing price`<br>3. `occupational employment and wage estimates in May 2017` <br>to visualize house price trends in states with time and estimate required work hours in a month to buy a house in different states. This tasks used `d3.js`, `vue.js`, `plotly.js`, `python`, `flask`, `pandas`, `SQLite`, `HTML`, and `CSS`.      |
|20| Tableau |    |
